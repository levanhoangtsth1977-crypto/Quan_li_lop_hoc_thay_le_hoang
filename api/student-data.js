...
let memoryCache = { data: null, storedAt: 0 };
let inFlight = null;
...
const byStudent = (list, sid, name, keys) => (Array.isArray(list) ? list : [])
  .filter(x => clean(x?.studentId) === sid || clean(x?.studentName) === name)
  .map(x => pick(x, keys));
  ...
  try {
    const response = await fetch(`${UPSTREAM}?action=get_all&_=${Date.now()}`, {
      ...
      headers: {
        ...
        'User-Agent': 'Mozilla/5.0 (compatible; StudentProfileProxy/3.0)'
      }
    });
...
async function getData() {
  const now = Date.now();
  if (memoryCache.data && (now - memoryCache.storedAt) < CACHE_TTL_MS) return memoryCache.data;
  if (inFlight) return inFlight;

  inFlight = (async () => {
    try {
      const data = await fetchUpstream();
      memoryCache = { data, storedAt: Date.now() };
      return data;
    } catch (error) {
      if (memoryCache.data && (Date.now() - memoryCache.storedAt) < STALE_TTL_MS) {
        console.warn('[student-data] using stale cache:', error?.message || error);

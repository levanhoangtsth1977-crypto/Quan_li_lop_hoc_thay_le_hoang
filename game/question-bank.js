/* TRIỆU PHÚ HỌC ĐƯỜNG — QUESTION BANK SYSTEM 3.7
   Nguồn duy nhất: Google Sheets / BO_CAU_HOI.
   Game lọc theo Môn + Chủ đề/Chương; không phụ thuộc HocKy.
*/
(function(){'use strict';
...
function status(t,ok){statusEl=document.getElementById('questionBankStatus');if(statusEl){statusEl.textContent=t;statusEl.dataset.state=ok?'ok':'loading';}}
...
function parse(data){
 ...
 var aliases={STT:['STT'],ID:['ID','MAID','QUESTIONID'],Mon:['MON','MONHOC','SUBJECT'],HocKy:['HOCKY','HOC KY','HOC KI','HOCKI','SEMESTER','KYHOC','HOC_KY'],Chuong:['CHUONG','CHUDE','CHUONGCHUDE','CHUDEBANG','CHAPTER','TOPIC'],Bo:['BO','SET'],MucDo:['MUCDO','MUC','LEVEL'],NhomKT:['NHOMKT'],CauHoi:['CAUHOI','QUESTION'],A:['A','DA','DAPANA'],B:['B','DB','DAPANB'],C:['C','DC','DAPANC'],D:['D','DD','DAPAND'],DapAn:['DAPAN','DAPANDUNG','CORRECTANSWER'],GiaiThich:['GIAITHICH','EXPLANATION'],Diem:['DIEM','POINTS'],ThoiGian:['THOIGIAN','TIME'],TrangThai:['TRANGTHAI','STATUS']};
 ...
 if(required.some(function(k){return map[k]<0;}))throw new Error('BO_CAU_HOI thiếu cột bắt buộc. Cần tối thiểu: ID | Mon | MucDo | CauHoi | A | B | C | D | DapAn');
 var list=rows.map(function(r){var c=r.c||[];function val(k){var i=map[k],cell=i>=0?c[i]:null;return cell?clean(cell.v!=null?cell.v:cell.f):'';}var st=val('TrangThai').toLowerCase();if(st&&['active','hoatdong','hoạt động','1','true','on'].indexOf(st)<0)return null;var opts=[val('A'),val('B'),val('C'),val('D')];if(!val('CauHoi')||opts.some(function(x){return !x;}))return null;var rawChuong=val('Chuong');return{id:val('ID'),subject:subject(val('Mon')),semester:val('HocKy'),chapter:rawChuong,set:val('Bo'),level:val('MucDo')||'M1',group:val('NhomKT'),question:val('CauHoi'),options:opts,correctAnswer:answerIndex(val('DapAn')),explanation:val('GiaiThich'),points:num(val('Diem'),1),time:num(val('ThoiGian'),30),status:val('TrangThai')||'ACTIVE',stt:val('STT')};}).filter(Boolean).filter(function(q){return q.id;});
 if(!list.length)throw new Error('BO_CAU_HOI không có câu hỏi Active hợp lệ');return list;
}
function finish(list){loading=false;ready=true;window.QuestionBank.ready=true;window.QuestionBank.count=list.length;window.QuestionBank.error=null;questions.splice(0,questions.length);Array.prototype.push.apply(questions,list);status('☁️ Ngân hàng Google: '+list.length+' câu • sẵn sàng lọc Môn/Chương',true);if(typeof window.initQuestionFilters==='function')window.initQuestionFilters();try{window.dispatchEvent(new Event('questionBankReady'));}catch(e){}if(pendingStart){pendingStart=false;setTimeout(function(){var b=document.querySelector('[data-action="start"]');if(b)b.click();},0);}}

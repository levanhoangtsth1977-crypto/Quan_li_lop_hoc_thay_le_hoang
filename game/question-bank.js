/* TRIỆU PHÚ HỌC ĐƯỜNG — QUESTION BANK SYSTEM 3.1 */
(function(){'use strict';
...
function norm(v){return clean(v).normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[\s_\-]+/g,'').toUpperCase();}
function num(v,d){var n=Number(v);return Number.isFinite(n)?n:d;}
function subject(v){var s=clean(v).toUpperCase();if(s==='TOAN'||s==='MATH'||s==='MATHEMATICS'||s==='➗ TOÁN')return'math';if(s==='TV'||s==='TIENGVIET'||s==='TIẾNG VIỆT'||s==='VIETNAMESE'||s==='📖 TIẾNG VIỆT')return'vietnamese';if(s==='KH'||s==='KHOA_HOC'||s==='KHOA HỌC'||s==='SCIENCE'||s==='🔬 KHOA HỌC')return'science';if(s==='LSDL'||s==='LS_DIA_LI'||s==='LSĐL'||s==='LICH SU & DIA LI'||s==='LỊCH SỬ & ĐỊA LÍ'||s==='HISTORY'||s==='HISTORY_GEOGRAPHY'||s==='🌏 LỊCH SỬ & ĐỊA LÍ')return'history';return s.toLowerCase();}
...
function parse(data){
 ...
 var aliases={STT:['STT'],ID:['ID','MAID','QUESTIONID'],Mon:['MON','MONHOC','SUBJECT'],HocKy:['HOCKY','HOC KY','SEMESTER'],Chuong:['CHUONG','CHUONGCHUDE','CHAPTER','TOPIC'],Bo:['BO','BỘ','SET'],MucDo:['MUCDO','MUC'],NhomKT:['NHOMKT'],CauHoi:['CAUHOI','QUESTION'],A:['A','DA','DAPANA'],B:['B','DB','DAPANB'],C:['C','DC','DAPANC'],D:['D','DD','DAPAND'],DapAn:['DAPAN','DAPANDUNG','CORRECTANSWER'],GiaiThich:['GIAITHICH','EXPLANATION'],Diem:['DIEM','POINTS'],ThoiGian:['THOIGIAN','TIME'],TrangThai:['TRANGTHAI','STATUS']};
 ...
 if(required.some(function(k){return map[k]<0;})&&rows.length){var first=(rows[0]&&rows[0].c)||[];var firstHeads=first.map(function(cell){return norm(cell&&cell.v);});var oldHeads=heads;heads=firstHeads;Object.keys(aliases).forEach(function(k){map[k]=idx(k);});if(required.every(function(k){return map[k]>=0;}))rows=rows.slice(1);else heads=oldHeads;}
...
function fail(err){loading=false;ready=false;window.QuestionBank.ready=false;window.QuestionBank.error=err;pendingStart=false;status('❌ Không tải được Google Sheet: '+(err&&err.message?err.message:'Lỗi kết nối'),false);}

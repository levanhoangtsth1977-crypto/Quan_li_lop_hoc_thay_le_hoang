...
window.GameData={
 ...
 loadStudents:function(){return fetch('../DANH_SACH_HOC_SINH_5C_2026_2027.json?v=2026.08.20',{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('Không tải được master roster');return r.json();}).then(function(data){roster=Array.isArray(data)?data:(Array.isArray(data.students)?data.students:[]);window.students=roster.slice();try{window.dispatchEvent(new CustomEvent('gameRosterReady',{detail:{count:roster.length}}));}catch(_){}return roster;});},
...
function bootRoster(){if(window.GameData.getStudents().length)return;window.GameData.loadStudents().catch(function(err){console.warn('GameData:',err);});}

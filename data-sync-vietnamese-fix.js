/* ============================================================
   AI / GPT SUMMARY FIX — LÊ HOÀNG
   VERSION 12.0 — ROBUST DATA RESOLUTION
   ============================================================ */
(function(){
'use strict';
if(window.__LH_AI_GPT_FIX_12__) return;
window.__LH_AI_GPT_FIX_12__ = true;
...
function render(){
  ...
  b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();latest=build();if(!latest.rows.length){view.style.display='block';view.innerHTML='<p>Chưa đọc được danh sách học sinh. Hãy mở menu Học sinh và kiểm tra dữ liệu đã đồng bộ.</p>';return;}const body=latest.rows.map(x=>`<tr><td>${x.stt}</td><td>${esc(x.name)}</td><td>${x.excused}</td><td>${x.absent}</td><td>${x.totalAbsent}</td><td>${x.violations}</td><td>${x.rewards}</td></tr>`).join('');view.innerHTML=`<div style="margin-bottom:8px"><strong>${latest.total.students} học sinh</strong> · Có phép ${latest.total.excused} · Không phép ${latest.total.absent} · Vi phạm ${latest.total.violations} · Khen thưởng ${latest.total.rewards}</div><table class="data-table" style="min-width:760px"><thead><tr><th>STT</th><th>Học sinh</th><th>Vắng có phép</th><th>Vắng không phép</th><th>Tổng vắng</th><th>Vi phạm</th><th>Khen thưởng</th></tr></thead><tbody>${body}</tbody></table>`;view.style.display='block';cp.disabled=false;op.disabled=false;window.showToast?.(`Đã tổng hợp ${latest.rows.length} học sinh.`,'success');});
  cp.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(latest)copyText(latest.prompt).then(ok=>window.showToast?.(ok?'Đã sao chép bảng + prompt.':'Không thể sao chép tự động; hãy chọn và sao chép thủ công.','warning'));});
...
function enforceClass(){try{if(typeof CLASS_CONFIG!=='undefined')CLASS_CONFIG.className='5A3';}catch(_){}try{const s=document.getElementById('classSelect');if(s)s.value='5A3';const h=document.getElementById('heroClass');if(h)h.textContent='Lớp 5A3';}catch(_){} }
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
window.addEventListener('google-sheets-data-ready',()=>{enforceClass();render();});
window.LH_AIGPT12={buildGPTReport:build,renderGPT:render,enforceClass};
})();
/* ============================================================
   ...
   GOOGLE API BRIDGE 5.2 — SAFE ROSTER SYNC + A-Z DISPLAY GUARD
   ============================================================ */
"use strict";
...
function cleanRosterForDisplay(list){
  if(!Array.isArray(list))return [];
  return sortStudentsAZForDisplay(
    list
      .map(normalizeStudentForBridge)
      .filter(function(s){return Boolean(s.id&&s.name);})
  );
}
...
function writeStudentsToLocal(list,source){
  ...
  var normalized=cleanRosterForDisplay(list);
  ...
  return{success:true,ok:true,count:normalized.length,source:source,sort:"A-Z-by-given-name",validOnly:true};
}
...
function syncStudentsVip(){
  return waitDataEngine().then(function(ready){if(!ready)throw new Error("Data Engine chưa sẵn sàng.");return fetchRemoteRoster();}).then(function(remote){
    ...
    window.__GOOGLE_CLASS_SYNC__={ok:true,count:remote.list.length,source:"google-sheets-live",fallbackUsed:false,autoRestore:false,sort:"A-Z-by-given-name",validOnly:true,at:new Date().toISOString()};
    refreshAllAfterSync();
...
function installReplaceStudentsApi(){
  ...
  window.replaceStudents=function(incoming,options){
    var list=cleanRosterForDisplay(incoming);
...
function installDisplayRosterGuard(){
  if(window.__QL_ROSTER_DISPLAY_GUARD_520__)return;
  var original=window.getStudentsSafe;
  if(typeof original!=="function")return;
  window.getStudentsSafe=function(){
    try{return cleanRosterForDisplay(original());}
    catch(error){console.warn("[GOOGLE BRIDGE] Display roster guard:",error);return[];}
  };
  window.__QL_ROSTER_DISPLAY_GUARD_520__=true;
}
function initializeGoogleApiBridge(){
  if(window.__GOOGLE_CLASS_BRIDGE_520__)return;
  window.__GOOGLE_CLASS_BRIDGE_520__=true;
  installReplaceStudentsApi();
  installDisplayRosterGuard();
  syncStudentsVip().then(function(result){if(typeof window.showToast==="function")window.showToast(result.ok?"Đã đọc danh sách học sinh từ Google Sheets: "+result.count+" em — chỉ hiển thị bản ghi hợp lệ, đã sắp xếp A–Z.":"Google chưa đọc được; dữ liệu hiện có được giữ nguyên.",result.ok?"success":"warning");});
}
...
(function(){
  ...
  window.__QL_VI_SYNC_FIX__={version:"5.2.0",translate:function(v){var k=String(v??"").trim().toLocaleLowerCase("vi");return TEXT[k]||String(v??"");}};
})();

 ...
 * - Xem hồ sơ mở trực tiếp trang hồ sơ canonical v5.
...
(function(){
  ...
  const PROFILE_PAGE='student-profile-v5.html';
  ...
  const getStudents=()=>{
    try{
      ...
      if(typeof window.getStudentsSafe==='function'){
        const b=window.getStudentsSafe();
  ...
  async function render(){
    ...
    body.querySelectorAll('[data-copy-student-link-fast]').forEach(btn=>{
      btn.addEventListener('click',async()=>{
        ...
        if(!url||url==='Đang tạo link…'){
          toast('Link chưa sẵn sàng, vui lòng chờ một chút.','warning');
          return;
        }
        try{
          await copy(url);
          toast('Đã sao chép đúng link của học sinh này.','success');
        }catch(e){
          toast(e?.message||'Không thể sao chép link.','error');
        }
      });
    });
  }

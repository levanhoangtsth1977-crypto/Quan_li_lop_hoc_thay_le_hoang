/* GOOGLE SHEETS MENU BRIDGE 5.2 — AUTHORITATIVE WEB APP + EVENT BINDING */
(function(){
'use strict';
if(window.__LH_GOOGLE_MENU_BRIDGE_520__)return;
window.__LH_GOOGLE_MENU_BRIDGE_520__=true;
...
const VERSION='5.2-AUTHORITATIVE-WEBAPP-EVENTS';
...
function replaceArray(name,list){const data=Array.isArray(list)?list:[];try{if(Array.isArray(window[name])){window[name].splice(0,window[name].length,...data);return data}}catch(_){}try{window[name]=data}catch(_){}return data}
function bindEvents(r){
 ...
 replaceArray('attendanceRecords',attendance);
 replaceArray('violationRecords',violations);
 replaceArray('rewardRecords',rewards);
 try{if(window.APP_DATA){window.APP_DATA.attendance=window.attendanceRecords;window.APP_DATA.violations=window.violationRecords;window.APP_DATA.rewards=window.rewardRecords}}catch(_){}
...
window.syncStudentsToGoogleSheet=async()=>({ok:false,error:'MASTER-5.2: danh sách học sinh lấy từ Google Sheets, không ghi đè từ Web'});

/* STUDENT PROFILE MENU 3.0 — SINGLE SOURCE STUDENT PICKER */
(function(){
'use strict';
if(window.__LH_STUDENT_PROFILE_MENU_30__)return;
window.__LH_STUDENT_PROFILE_MENU_30__=true;
const $=id=>document.getElementById(id);
const clean=v=>String(v??'').trim().replace(/\s+/g,' ');
const esc=v=>String(v??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;');
const SELECTED_ID='lhProfileSelectedStudentId';
let fallbackRoster=[];
function uniqueRoster(list){
 const seen=new Set();
 return (Array.isArray(list)?list:[]).filter(s=>{
   const id=clean(s?.id||s?.studentCode);
   const name=clean(s?.name);
   if(!id||!name||seen.has(id))return false;
   seen.add(id);
   return true;
 }).map((s,i)=>({...s,id:clean(s.id||s.studentCode),_stt:Number(s.stt)||i+1})).sort((a,b)=>a._stt-b._stt);
}
function students(){
 const g=window.GOOGLE_SHEETS_STUDENTS;
 const s=window.students;
 const primary=Array.isArray(g)&&g.length?g:(Array.isArray(s)&&s.length?s:[]);
 const roster=uniqueRoster(primary.length?primary:fallbackRoster);
 return roster;
}
function getEvents(tab,name){const g=window.GOOGLE_SHEET_DATA?.tabs?.[tab];return Array.isArray(g)?g:(Array.isArray(window[name])?window[name]:[])}
async function loadFallback(){
 ...
 try{
   ...
   fallbackRoster=uniqueRoster(data?.students||[]);
 }catch(e){fallbackRoster=[]}
 return fallbackRoster;
}
function currentId(){return clean($(SELECTED_ID)?.value)}
function setSelected(id){const h=$(SELECTED_ID);if(h)h.value=clean(id)}
function closePicker(){const p=$('lhProfilePickerPanel'),a=$('lhProfilePickerArrow');if(p)p.hidden=true;if(a)a.textContent='▾'}
function resetDetail(){const box=$('lhProfileDetail');if(box)box.innerHTML='<div class="empty-state"><span class="empty-icon"><i class="fa-solid fa-user-graduate"></i></span><strong>Chọn một học sinh</strong><p>Hồ sơ chi tiết sẽ hiển thị tại đây.</p></div>'}
function selectProfile(id){
 const sid=clean(id);const s=students().find(x=>x.id===sid);if(!s)return;
 setSelected(sid);
 $('lhProfilePickerLabel').textContent=s.name;
 closePicker();
 renderProfile(sid);
}
function renderPicker(query=''){
 ...
 const searchWrap=document.createElement('div');searchWrap.className='lh-profile-picker-search';searchWrap.innerHTML='<i class="fa-solid fa-magnifying-glass"></i><input id="lhProfilePickerSearch" type="search" placeholder="Tìm học sinh..." autocomplete="off">';panel.appendChild(searchWrap);
 ...
 const draw=q=>{
   ...
   const filtered=roster.filter(s=>!key||s.name.toLocaleLowerCase('vi').includes(key));
 ...
 const input=$('lhProfilePickerSearch');
 if(input)input.addEventListener('input',()=>draw(input.value));
 draw(query);
}
function syncPicker(){
 const label=$('lhProfilePickerLabel');if(!label)return;
 const roster=students();
 const sid=currentId();
 const selected=roster.find(s=>s.id===sid);
 if(selected)label.textContent=selected.name;else{setSelected('');label.textContent='Chọn học sinh'}
}
function ensureMenu(){
 ...
 const b=document.createElement('button');b.type='button';b.className='menu-item';b.id='lhStudentProfileMenu';b.dataset.page='student-profiles';b.innerHTML='<i class="fa-solid fa-id-card"></i><span>Hồ sơ học sinh</span>';
...
function ensurePage(){
 ...
 resetDetail();syncPicker();renderList($('lhProfileSearch')?.value||'');
}
function renderList(q=''){
 const box=$('lhProfileStudentList');if(!box)return;
 const key=clean(q).toLocaleLowerCase('vi');
 const list=students().filter(s=>!key||s.name.toLocaleLowerCase('vi').includes(key));
...
function renderProfile(id){
 const sid=clean(id),s=students().find(x=>x.id===sid),box=$('lhProfileDetail');if(!s||!box)return;

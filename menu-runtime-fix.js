/* MENU RUNTIME FIX 8.35 — safe menu rescue + isolated form/link fixes */
(function(){
  'use strict';
  if(window.__MENU_RUNTIME_FIX_835__) return;
  window.__MENU_RUNTIME_FIX_835__=true;
  ...
  function showPageFallback(page){
    ...
    document.querySelectorAll('[data-page-section]').forEach(section=>{const active=section===target;section.hidden=!active;section.classList.toggle('active',active)});
    document.querySelectorAll('.main-menu .menu-item[data-page]').forEach(item=>item.classList.toggle('active',item.getAttribute('data-page')===String(page)));
    const title=document.getElementById('pageTitle');if(title)title.textContent=TITLES[page]||String(page);closeMobile();
  ...
  function menuRescue(event){
    ...
    const menu=t.closest('.main-menu .menu-item[data-page]');if(!menu)return;
    const page=menu.getAttribute('data-page');if(!page)return;
    const router=typeof window.navigateToPage==='function'?window.navigateToPage:(window.LopHocApp&&typeof window.LopHocApp.navigateToPage==='function'?window.LopHocApp.navigateToPage:null);
    if(router){try{const ok=router(page);if(ok!==false){event.preventDefault();event.stopImmediatePropagation()}return}catch(_){} }
    if(showPageFallback(page)){event.preventDefault();event.stopImmediatePropagation()}
  }
  ...
  function boot(){
    ...
    /* Khôi phục danh sách học sinh trong 2 form, không can thiệp router. */
    loadOnce('behavior-form-select-repair.js?v=20260913.1','data-lh-behavior-form-select-repair-v10');
    /* Tăng tốc tạo link cá nhân và bảo đảm nút sao chép dùng đúng studentId của dòng. */
    loadOnce('student-links-fast-fix.js?v=20260913.1','data-lh-student-links-fast-fix-v10');
  }

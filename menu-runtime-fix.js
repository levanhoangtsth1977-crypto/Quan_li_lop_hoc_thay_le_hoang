/* MENU RUNTIME 8.65 — one canonical SAVE path; legacy form handlers blocked */
(function(){
  'use strict';
  if(window.__MENU_RUNTIME_FIX_865__)return;
  window.__MENU_RUNTIME_FIX_865__=true;
  ...
  /* HARD SAVE GATE: legacy Vi phạm/Khen thưởng submit handlers never run. */
  async function canonicalFormSave(form){
    ...
    const record={
      id:`${isV?'VIO':'REW'}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,8)}`,
      studentId,
      studentName:S(student?.name),
      date,
      type,
      note:S(form.querySelector(isV?'#violationNote':'#rewardNote')?.value),
      createdAt:new Date().toISOString(),
      updatedAt:new Date().toISOString()
    };
    if(isV){
      record.level=S(form.querySelector('#violationLevel')?.value)||'light';
      record.status=S(form.querySelector('#violationStatus')?.value)||'monitoring';
      record.action=S(form.querySelector('#violationAction')?.value);
    }else{
      record.formType=S(form.querySelector('#rewardFormType')?.value)||'praise';
    }
    try{
      if(typeof window.LH_MASTER_CRUD?.saveMaster!=='function')throw Error('Bộ lưu Master chưa sẵn sàng. Vui lòng thử lại sau.');
      const sheet=isV?'VI_PHAM':'KHEN_THUONG';
      const saved=await window.LH_MASTER_CRUD.saveMaster(sheet,record);
      try{window.LH_MASTER_CRUD.render(sheet)}catch(_){}
      try{form.reset()}catch(_){}
      if(isV){const d=form.querySelector('#violationDate');if(d)d.value=today()}else{const d=form.querySelector('#rewardDate');if(d)d.value=today()}
      const modal=form.closest('.modal');if(modal)modal.hidden=true;
      toast(isV?'Đã lưu vi phạm vào Google Sheets.':'Đã lưu khen thưởng vào Google Sheets.','success');
    }catch(e){toast('Lưu thất bại: '+S(e?.message||e),'error')}
  }
  function formSaveGate(e){
    const f=e.target instanceof HTMLFormElement?e.target:null;
    if(!f||!['violationForm','rewardForm'].includes(S(f.id)))return;
    e.preventDefault();
    e.stopPropagation();
  ...
  function boot(){
    ...
    loadOnce('master-crud-ui.js?v=20260915.9','data-lh-master-crud-ui-v10');

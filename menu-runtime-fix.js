/* MENU RUNTIME FIX 8.34 — safe menu rescue + runtime dependencies */
(function(){
  'use strict';
  ...
  /*
   * MENU RESCUE an toàn:
   * Ưu tiên router chính của script.js. Chỉ dùng fallback khi router chưa sẵn sàng.
   * Tuyệt đối không chặn render của router chính.
   */
  function menuRescue(event){
    ...
    const router=typeof window.navigateToPage==='function'
      ? window.navigateToPage
      : (window.LopHocApp && typeof window.LopHocApp.navigateToPage==='function'
          ? window.LopHocApp.navigateToPage
          : null);

    if(router){
      try{
        const ok=router(page);
        if(ok!==false){

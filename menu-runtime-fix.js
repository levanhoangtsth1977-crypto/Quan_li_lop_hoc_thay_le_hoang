/* MENU RUNTIME FIX 8.7 — lean statistics enhancer + isolated loaders
   Navigation is owned by script.js + UI ACTION PATCH 2.1.0.
   This module must never install another menu router or repeated timer loaders.
*/
(function(){
  'use strict';
  ...
  function install(){
    document.addEventListener('click',function(e){const tab=e.target&&e.target.closest?e.target.closest('[data-lh-stat-tab]'):null;if(tab){e.preventDefault();e.stopPropagation();render(tab.dataset.lhStatTab);}},false);
    window.addEventListener('google-sheets-data-ready',function(){if(host()&&host().classList.contains('active'))render('DIEM_DANH');},false);
  }
  ...
  function boot(){
    ...
    loadOnce('attendance-status-final-fix.js?v=7.0.0','data-lh-attendance-status-final-fix');
  }

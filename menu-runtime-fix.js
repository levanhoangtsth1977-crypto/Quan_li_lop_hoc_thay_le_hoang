/* MENU RUNTIME FIX 8.15 — lean runtime
   ...
   Learning data remains available from SMAS import for statistics/AI.
   Behavior tables/AI summary use one canonical module only.
*/
(function(){
  'use strict';
  ...
  function loadOnce(src,attr){const old=document.querySelector('script[data-lh-behavior-records-ai-summary]');if(old){old.remove()}const s=document.createElement('script');s.src=src;s.async=false;s.setAttribute(attr,'1');document.head.appendChild(s)}
  function boot(){
    install();
    ...
    loadOnce('behavior-records-ai-summary.js?v=1.2.0','data-lh-behavior-records-ai-summary');
  }

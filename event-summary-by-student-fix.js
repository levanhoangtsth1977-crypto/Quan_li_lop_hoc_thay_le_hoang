...
function studentName(id){const sid=clean(id);try{if(typeof window.getStudentById==='function'){const s=window.getStudentById(sid);if(s&&s.name)return s.name}}catch(_){}const s=students().find(x=>clean(x.id)===sid);return s?s.name:'Học sinh'}

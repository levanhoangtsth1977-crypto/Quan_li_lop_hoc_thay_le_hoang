/* BEHAVIOR SAVE CANONICAL 2.6 — iframe POST to Apps Script */
(function(){
'use strict';
if(window.__LH_BEHAVIOR_SAVE_CANONICAL_26__)return;
window.__LH_BEHAVIOR_SAVE_CANONICAL_26__=true;
...
function values(form,isV){
  ...
  if(isV)out.extra={level:S(field(form,['#violationLevel','select[name="violationLevel"]'])?.value)||'light',status:S(field(form,['#violationStatus','select[name="violationStatus"]'])?.value)||'monitoring',action:S(field(form,['#violationAction','select[name="violationAction"]'])?.value)};
...
function localPush(sheet,record){
  ...
  const arr=window[name],i=arr.findIndex(x=>S(x?.id)===S(record.id));
...
function iframePost(sheet,record){
  return new Promise((resolve,reject)=>{
    const iframe=document.createElement('iframe');
    const form=document.createElement('form');
    const target='LH_POST_'+Date.now()+'_'+Math.random().toString(36).slice(2);
    ...
    add('action',sheet==='VI_PHAM'?'saveViolation':'saveReward');
    Object.entries(record).forEach(([k,v])=>add(k,v));
    ...
    const timer=setTimeout(()=>{if(done)return;done=true;cleanup();reject(Error('Google Apps Script không xác nhận được lượt gửi.'))},12000);
...
function jsonp(action,params,timeout=8000){
  return new Promise((resolve,reject)=>{
    const cb='LH26_'+Date.now()+'_'+Math.random().toString(36).slice(2);
...
async function save(form,isV,button){
  if(!form||form.dataset.lhCanonicalSaving26==='1')return;
  form.dataset.lhCanonicalSaving26='1';
  ...
  try{
    ...
    for(const student of chosen){
      ...
      localPush(sheet,rec);
      try{await iframePost(sheet,rec);sent++;if(await verifyCloud(sheet,rec))verified++}catch(e){console.warn('[LH26]',e)}
    }
    rerender();
    if(sent===chosen.length&&verified===chosen.length)toast(`Đã lưu ${sent} học sinh vào Google Sheets.`,'success');
    else if(sent===chosen.length)toast(`Đã gửi ${sent} học sinh lên Google Sheets; đang chờ Google xác minh.`,'success');
    else if(sent>0)toast(`Đã gửi ${sent}/${chosen.length} học sinh lên Google Sheets.`,'warning');
    else throw Error('Google Apps Script không nhận yêu cầu POST.');
  ...
  finally{form.dataset.lhCanonicalSaving26='';if(button){button.disabled=false;button.innerHTML=old}}
}
...
function onSubmit(e){
  const form=e.target instanceof HTMLFormElement?e.target:null;if(!form||!/^((violation)|(reward))Form$/.test(form.id))return;
  e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
  const b=form.querySelector('button[type="submit"]')||[...form.querySelectorAll('button')].find(x=>/\blưu\b/i.test(S(x.textContent)));
  save(form,form.id==='violationForm',b);
}

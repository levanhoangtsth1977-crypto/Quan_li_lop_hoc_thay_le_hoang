...
function renderStatus(){
 ...
 status.querySelectorAll('[data-smas-analyze]').forEach(btn=>btn.addEventListener('click',()=>openSMASAnalyzer(btn.dataset.smasAnalyze)));
 ...
 const analyses=loadSMASAnalyses();
 const total=data.reduce((n,r)=>n+(Number(r.totalRows)||0),0),students=buildEvidence().length;
 p.querySelector('#lhSmasSummary').innerHTML=`<div style="display:flex;gap:14px;flex-wrap:wrap;color:#334155"><span><strong>${data.length}/4</strong> giai đoạn đã tải</span><span><strong>${total}</strong> dòng dữ liệu</span><span><strong>${students}</strong> học sinh có dữ liệu định kỳ</span><span><strong>${analyses.length}/4</strong> giai đoạn đã phân tích chuẩn</span></div>`;
}
...
function loadSMASAnalyses(){try{const x=JSON.parse(localStorage.getItem('LH_SMAS_ANALYSIS_2026_2027')||'[]');return Array.isArray(x)?x:[]}catch(e){return[]}}
function saveSMASAnalysis(period,payload){const list=loadSMASAnalyses().filter(x=>x.period!==period);list.push({period,periodLabel:LABELS[period]||period,receivedAt:new Date().toISOString(),payload});localStorage.setItem('LH_SMAS_ANALYSIS_2026_2027',JSON.stringify(list));window.dispatchEvent(new CustomEvent('lh-smas-analysis-updated',{detail:{period,payload}}))}
function openSMASAnalyzer(period){const existing=document.getElementById('lhSMASAnalyzerModal');if(existing)existing.remove();const m=document.createElement('div');m.id='lhSMASAnalyzerModal';m.style.cssText='position:fixed;inset:0;z-index:100000;background:rgba(15,23,42,.72);display:flex;align-items:center;justify-content:center;padding:10px';m.innerHTML='<div style="width:min(1500px,100%);height:min(96vh,1100px);background:#fff;border-radius:16px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 25px 70px rgba(0,0,0,.35)"><div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;border-bottom:1px solid #e2e8f0;background:#fff"><strong>📊 Phân tích bảng tổng hợp SMAS — '+esc(LABELS[period]||period)+'</strong><button type="button" id="lhSMASAnalyzerClose" class="button secondary">✕ Đóng</button></div><iframe title="Phân tích SMAS" src="/smas-5a3/?embedded=1&period='+encodeURIComponent(period)+'" style="width:100%;height:100%;border:0;flex:1;background:#f4f7fb"></iframe></div>';document.body.appendChild(m);m.querySelector('#lhSMASAnalyzerClose').onclick=()=>m.remove();m.addEventListener('click',e=>{if(e.target===m)m.remove()})}
window.getSMASAnalysisData=loadSMASAnalyses;
window.addEventListener('message',e=>{if(e.origin!==location.origin||!e.data||e.data.type!=='LH_SMAS_ANALYSIS_READY')return;const period=String(e.data.period||'').trim();if(!PERIODS.includes(period))return;saveSMASAnalysis(period,e.data.payload||{});renderStatus();toast('Đã nhận kết quả phân tích chuẩn từ SMAS cho '+LABELS[period]+'.','success')});

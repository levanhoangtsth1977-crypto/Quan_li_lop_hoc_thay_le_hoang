/* GOOGLE SHEETS RECORDS BRIDGE 2.4 — AUTHORITATIVE EVENT SYNC / DELETE SAFE */
(function(){
'use strict';
if(window.__LH_GOOGLE_RECORDS_BRIDGE_240__)return;
window.__LH_GOOGLE_RECORDS_BRIDGE_240__=true;
...
function applyEvents(d){if(!d||d.ok!==true)return false;replace('attendanceRecords',(d.DIEM_DANH||[]).filter(r=>clean(r.studentId)&&!present(r)));replace('violationRecords',(d.VI_PHAM||[]).filter(r=>clean(r.studentId)));replace('rewardRecords',(d.KHEN_THUONG||[]).filter(r=>clean(r.studentId)));try{if(typeof window.syncAppDataReferences==='function')window.syncAppDataReferences()}catch(_){}try{if(typeof window.renderAttendance==='function')window.renderAttendance()}catch(_){}try{if(typeof window.renderViolations==='function')window.renderViolations()}catch(_){}try{if(typeof window.renderRewards==='function')window.renderRewards()}catch(_){}window.GOOGLE_SHEET_EVENT_DATA={version:'2.4',mode:'REPLACE',loadedAt:new Date().toISOString(),tabs:d};return true}
...
function install(){['saveAttendanceRecord','addViolation','deleteViolation','addReward','deleteReward'].forEach(n=>{try{if(typeof window[n]!=='function'||window[n].__lh240)return;const o=window[n];const w=function(){const r=o.apply(this,arguments);setTimeout(async()=>{await push();await pull()},200);return r};w.__lh240=true;w.__lhOriginal=o;window[n]=w}catch(_){}})}
async function boot(){install();await pull();install();if(window.__LH240_TIMER__)clearInterval(window.__LH240_TIMER__);window.__LH240_TIMER__=setInterval(install,1000)}

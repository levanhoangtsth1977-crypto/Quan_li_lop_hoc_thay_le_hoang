/* GOOGLE SHEETS RECORDS BRIDGE 1.1
   - HOC_SINH remains the full authoritative roster.
   - DIEM_DANH stores only excused/absent records in Google Sheets.
   - VI_PHAM and KHEN_THUONG store only actual records.
   - Event arrays from data.js are global lexical bindings, so access them safely.
*/
(function(){
  'use strict';
  if(window.__LH_GOOGLE_RECORDS_BRIDGE_110__)return;
  window.__LH_GOOGLE_RECORDS_BRIDGE_110__=true;
  ...
  function post(payload){
    return new Promise((resolve,reject)=>{
      ...
      let done=false;const finish=ok=>{if(done)return;done=true;setTimeout(()=>{iframe.remove();form.remove()},500);ok?resolve({ok:true}):reject(Error('Không gửi được dữ liệu Google Sheets'))};
      iframe.onload=()=>finish(true);try{form.submit()}catch(e){finish(false)}setTimeout(()=>finish(true),12000);
    });
  }

  function idOf(r){return clean(r&&r.id)}
  function mergeById(target,incoming){
    const map=new Map(target.map(r=>[idOf(r),r]));
    (Array.isArray(incoming)?incoming:[]).forEach(r=>{const id=idOf(r);if(!id)return;const old=map.get(id);if(old)Object.assign(old,r);else target.push(r)});
  }

  function snapshot(){
    return {
      DIEM_DANH:records('attendanceRecords').filter(r=>!/^present$/i.test(clean(r.status))),
  ...
  function pullIntoApp(data){
    ...
    mergeById(records('attendanceRecords'),data.DIEM_DANH||[]);
    mergeById(records('violationRecords'),data.VI_PHAM||[]);
    mergeById(records('rewardRecords'),data.KHEN_THUONG||[]);
    ...
    window.GOOGLE_SHEET_EVENT_DATA={version:'1.1',loadedAt:new Date().toISOString(),tabs:data};
    return true;
  }

  async function pull(){try{const data=await jsonp('get_events');pullIntoApp(data);return data}catch(e){console.warn('[GOOGLE RECORDS] pull:',e.message);return null}}

  async function push(){
    if(syncing)return{ok:false,busy:true};syncing=true;
    try{const result=await post({action:'sync_events',records:snapshot()});window.GOOGLE_SHEET_EVENT_SYNC={ok:true,at:new Date().toISOString(),result};return result}
    catch(e){window.GOOGLE_SHEET_EVENT_SYNC={ok:false,at:new Date().toISOString(),error:e.message};console.warn('[GOOGLE RECORDS] push:',e.message);return{ok:false,error:e.message}}
  ...
  function wrap(name){
    ...
    function wrapped(){const result=original.apply(this,arguments);setTimeout(push,100);return result}
    wrapped.__lhWrapped=true;wrapped.__lhOriginal=original;window[name]=wrapped;
  }
  function installWrappers(){['saveAttendanceRecord','addViolation','deleteViolation','addReward','deleteReward'].forEach(wrap);installed=true}

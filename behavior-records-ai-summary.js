/* BEHAVIOR RECORDS + AI SUMMARY 1.1
 * Detailed tables for Vi phạm and Khen thưởng, sourced only from existing Data Engine records.
 * Creates the Vi phạm table when the static HTML does not contain one.
 * AI reads the exact same records; no data mutation.
 */
(function(){
  'use strict';
  if(window.__LH_BEHAVIOR_RECORDS_AI_SUMMARY__) return;
  window.__LH_BEHAVIOR_RECORDS_AI_SUMMARY__=true;
  ...
  function rows(kind){return getRecords(kind).slice().sort((a,b)=>S(b.date||b.createdAt).localeCompare(S(a.date||a.createdAt))).map((r,i)=>{const f=kind==='violation'?violationFields(r):rewardFields(r);return {...r,...f,studentName:nameOf(r.studentId),_stt:i+1}})}
  function page(id){return document.getElementById(id)||document.querySelector(`[data-page-section="${id.replace('page-','')}"]`)}
  function ensureViolationTable(p){
    if(!p)return null;
    let body=p.querySelector('#violationTableBody');
    if(body)return body;
    const existing=p.querySelector('.info-banner.warning');
    const wrap=document.createElement('div');wrap.className='table-container';wrap.id='lhViolationTableWrap';
    wrap.innerHTML='<table class="data-table" id="violationTable"><thead><tr><th>STT</th><th>Họ tên học sinh</th><th>Ngày vi phạm</th><th>Nội dung vi phạm</th><th>Hình thức xử lý</th><th>Thao tác</th></tr></thead><tbody id="violationTableBody"></tbody></table>';
    if(existing&&existing.parentNode)existing.parentNode.insertBefore(wrap,existing.nextSibling);else p.appendChild(wrap);
    return wrap.querySelector('#violationTableBody');
  }
  function renderTable(kind){
    const p=page(kind==='violation'?'page-violations':'page-rewards');if(!p)return;
    const selector=kind==='violation'?'#violationTableBody':'#rewardTableBody';
    let body=p.querySelector(selector);
    if(kind==='violation')body=ensureViolationTable(p);
    if(!body)return;
  ...
  function refresh(){renderTable('violation');renderTable('reward');renderAI();}

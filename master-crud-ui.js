/* MASTER CRUD UI — SINGLE SOURCE OF TRUTH v13
 * Canonical Google Sheets CRUD for the class-management menus.
 * Every successful write is followed by a fresh Google Sheets read so all
 * menus render from the same authoritative arrays instead of stale local copies.
 */
(function(){
'use strict';
if(window.__LH_MASTER_CRUD_UI_V13__)return;
window.__LH_MASTER_CRUD_UI_V13__=true;
...
async function saveModal(modal,b,isV){if(modal.dataset.lhMasterSavingV13==='1')return;modal.dataset.lhMasterSavingV13='1';b.disabled=true;const old=b.innerHTML;b.innerHTML='Đang lưu...';try{const v=readModal(modal,isV),sheet=isV?'VI_PHAM':'KHEN_THUONG';let ok=0;for(const studentId of v.students){const r={id:makeId(isV?'VIO':'REW'),studentId,studentName:(window.students||[]).find(s=>S(s.id)===studentId)?.name||'',date:v.date,type:v.type,note:v.note,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};if(isV){r.level=S(modal.querySelector('#eLevel,#violationLevel')?.value)||'light';r.status=S(modal.querySelector('#eStatus,#violationStatus')?.value)||'monitoring';r.action=S(modal.querySelector('#eAction,#violationAction')?.value)}else r.formType=S(modal.querySelector('#eForm,#rewardFormType')?.value)||'praise';await saveMaster(sheet,r);ok++}modal.remove();toast(`Đã lưu ${ok} học sinh vào Google Sheets.`,'success')}catch(e){toast('Lưu thất bại: '+S(e?.message||e),'error')}finally{modal.dataset.lhMasterSavingV13='';b.disabled=false;b.innerHTML=old}}
...
window.addEventListener('submit',clickCapture,true);
window.LH_MASTER_CRUD={saveMaster,deleteMaster,render,version:'13'};
})();
